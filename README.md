...
看了么通过 Android MediaStore 读取本机媒体库，帮助用户快速筛选照片和视频、管理收藏、放入 App 内部回收站，并在二次确认后删除媒体内容。它不是云相册，也不是社交相册，而是一个专注于本地媒体整理的工具。
...
| 模块         | 功能                                                       |
| ------------ | ---------------------------------------------------------- |
| 图片整理     | 滑动筛选、收藏、加入回收站、重新随机、排除文件夹、点击预览 |
| 视频整理     | 视频预览、上下切换、进度控制、收藏、加入回收站、排除文件夹 |
| 收藏与回收站 | 集中管理已收藏和待删除媒体，支持恢复与二次确认删除         |
| 往年今日     | 按日期重新发现历史照片和视频                               |
...
Windows 用户也可以使用 `scripts/windows/` 下的一键构建脚本。
